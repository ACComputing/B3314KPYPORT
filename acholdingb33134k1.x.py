...
def handle_movement(dt):
    ...
    # --- Puppycam: Mouse Look (dt‑aware for consistent turning) ---
    if pygame.event.get_grab():
        mouse_dx, mouse_dy = pygame.mouse.get_rel()
        sens_x = CFG["mouse_sensitivity_x"]
        if CFG["invert_x"]:
            mouse_dx = -mouse_dx
        player_angle += mouse_dx * sens_x * dt * 60
        player_angle += gp_lx * 0.05 * dt * 60

    # Keyboard look fallback
    if keys[pygame.K_LEFT]:
        player_angle -= 0.05 * dt * 60
    ...
    # Direction vectors
    dx_fwd = math.cos(player_angle) * move_speed
    dy_fwd = math.sin(player_angle) * move_speed
    dx_str = math.cos(player_angle + math.pi / 2) * move_speed
    dy_str = math.sin(player_angle + math.pi / 2) * move_speed

    # Desired acceleration (units per second)
    ...
    # Physics (dt‑aware)
    if player_on_ice:
        # Ice: exponential deceleration + acceleration scaled by dt
        friction = 2.0      # per second
        decel_factor = 0.9 ** (dt * 60)   # per frame at 60fps
        player_vel_x = player_vel_x * decel_factor + accel_x * dt * friction
        player_vel_y = player_vel_y * decel_factor + accel_y * dt * friction
    else:
        # Normal ground: direct velocity = acceleration
    ...
    # Conveyor push (scale by dt)
    if surf == SURFACE_CONVEY:
        player_x += math.cos(time.time()) * 0.03 * dt * 60
        player_y += math.sin(time.time()) * 0.03 * dt * 60
